#!/usr/bin/env python3
"""One-off deep backfill: extend the transfers/ and transfers_in/ shard caches
back to CUTOFF.
...
def deep(base_url, path):
    shards.migrate_legacy(path)
    cache = shards.load(path)
    ...
    while True:
        ...
        time.sleep(0.15)
    merged = sorted([shards.slim(i) for i in got] + cache, key=lambda i: i["timestamp"], reverse=True)
    shards.save(path, merged)
    print(path, "done:", len(got), "new,", len(merged), "total, oldest", merged[-1]["timestamp"][:19], flush=True)

deep(BASE, os.path.join(HERE, "transfers"))
deep(BASE.replace("filter=from", "filter=to"), os.path.join(HERE, "transfers_in"))
